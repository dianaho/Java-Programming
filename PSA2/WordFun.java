...
public class WordFun
{
  ...
  public int scrabbleScore( String word )
  {
    ...
    // Loop through word to get each character
    for ( int i = 0; i < word.length(); i++ )
    {
      char c = word.charAt( i );
      
      // Finding the score at letter c
      int individualScore = letterScore( c );
      
      // Totaling up individual letter scores
      totalScore = individualScore + totalScore;
    }
    return totalScore;
  }
  
  /** Encrypts a string by rotating each character by the amount rotation 
    * declared by int
    * @param s The string to encrypt
    * @param rotation The amount to rotate each letter of the string by
...
//    System.out.println(result);
  ...
  /** Encrypts a single character using the int given
    ...
    * @param number The amount to rotate the character by
  ...
  public void encryptFile(String infile, String outfile, int rotation) throws IOException
  {
    ...
    File sourceFile = new File( infile );  // Get infile
    Scanner input = new Scanner( sourceFile );  // Read from infile
 
    // Create a PrintWriter for output    
    File targetFile = new File( outfile ); // Get outfile
    PrintWriter output = new PrintWriter( targetFile );  // Write to outfile
  ...
  public static void main( String[] args ) throws IOException
  {
    ...
    System.out.println("Score for 'diana' = "+f.scrabbleScore("diana") + " ...(should be 6)");
    System.out.println("\n");
